...
let time f =
    ...
    for i = 1 to 3 do f()
    (System.DateTime.Now - start).TotalMilliseconds / 3.00

let charting =
    Chart.Combine
        [
             Chart.Line( [ for i in 0..5..200 -> (i, time(fun () -> CYK rl1 (stringGener nonterm1 i) "S" |> ignore))], "CYK", Color = System.Drawing.Color.Green)
             ...
             Chart.Line( [ for i in 0..5..200 -> (i, time(fun () -> CYKParallelGPGPU rl1 (stringGener nonterm1 i) "S" |> ignore))], "CYKParallelGPGPU", Color = System.Drawing.Color.Aqua)
        ]
